...
async def process_chat(
    ...
    try:
        gguf_path = _resolve_generation_path(model)
    ...
    history_limit = settings.get("chatHistoryLimit", 6)
    ...
    try:
        ...
        if memory_enabled:
            ...
            mem_summary = user_memory.get_memory_summary()
            if mem_summary:
...
"""
        ...
        if not llama_runtime.is_loaded or (llama_runtime._model_path and Path(llama_runtime._model_path).resolve() != gguf_path.resolve()) or n_ctx_differs:
            yield {"token": "⚡ *Loading GGUF model...*\n\n", "done": False, "conversation_id": conversation_id}
        ...
        if actual_vision:
            ...
            async for token in llama_runtime.generate_chat_stream(
                ...
                yield {"token": token, "done": False, "conversation_id": conversation_id}
        else:
